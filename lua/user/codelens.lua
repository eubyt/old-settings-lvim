...
M.code_lens_available = function(cursor)
    ...
    for _, l in ipairs(vim.lsp.codelens.get(0)) do
        table.insert(codelens_actions, {
            start = l.range.start,
            finish = l.range["end"]
        })
    end
    for _, action in ipairs(codelens_actions) do
        if action.start.line <= cursor[1] and cursor[1] <= action.finish.line and action.start.character <= cursor[2] and
            cursor[2] <= action.finish.character then
            return true
        end
    end
    return false
...
M.handler_factory = function(line, bufnr)
    ...
    local function code_action_handler(responses)
        local ignored_messages = {"Extract Block", "Extract Block To File", "Extract Function",
                                  "Extract Function To File", "Extract Variable", "Inline Variable"}
...
M.update_sign = function(priority, old_line, new_line, bufnr, text)
    ...
    if new_line and (vim.b.lightbulb_line ~= new_line) then
        vim.fn.sign_place(new_line, SIGN_GROUP, SIGN_NAME, bufnr, {
            lnum = new_line,
            priority = priority
        })
        -- Update current lightbulb line
