...
M.config = function()
    ...
    local plugin_count = {
        type = "text",
        val = kind.cmp_kind.Module .. " " .. string.format("% 3d", plugins) .. " plugins  v" .. vim.version().major ..
    ...
    local buttons = {
        type = "group",
        val = {button("f", " " .. kind.cmp_kind.Folder .. " Explore",
            "<cmd>lua require('lvim.core.telescope.custom-finders').find_project_files()<CR>"),
               ...
               button("g", " " .. kind.icons.git .. " Git Status",
    ...
    local opts = {
        layout = {{
            ...
            type = "padding",
            val = 2
        }, section.heading, section.plugin_count, {
            type = "padding",
            val = 1
        }, -- section.top_bar,
